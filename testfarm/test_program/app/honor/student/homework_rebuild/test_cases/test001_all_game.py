...
class PlayAllGame(unittest.TestCase):
    ...
    def test_all_game(self, hw_name, bank_type):
        if self.home.wait_check_home_page():
            ...
            if self.home.wait_check_home_page():
                self.home.click_hk_tab(index=2)  # 进入习题
                bank_list = self.library.enter_into_game(hw_name, bank_type)   # 获取大题元素与名称
                for i in range(len(bank_list)):                      # 遍历点击所有该题型的小题
                    if self.library.wait_check_bank_list_page():               # 题目列表页面检查点
                        bank_name = bank_list[i].text
