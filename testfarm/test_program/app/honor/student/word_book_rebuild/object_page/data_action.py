...
class DataActionPage(BasePage):
    ...
    def change_word_date(self):
        ...
        self.mysql.update_word_date(str(date[0]), stu_id[0][0], gv.LEVEL)
        self.mysql.update_word_record(str(date[1]), stu_id[0][0])  # 单词去重，更改record的create时间
    ...
    @teststep
    def delete_word_homework(self):
        # 删除学生单词作业
        self.mysql.delete_student_word_homework(gv.STU_ID)
        # 删除学生单词作业记录
        self.mysql.delete_student_homework_record(gv.STU_ID)
        # 删除学生单词错题记录
        self.mysql.delete_student_homework_wrong(gv.STU_ID)
        # 删除单词作业 与书籍对应数据
        self.mysql.delete_homework_label_map_info(gv.STU_ID)
    ...
    @teststep
    def get_student_word(self):
        """获取学生单词 单词按照布置时间逆序获取"""
        all_word = {}
        teacher_label_id = self.mysql.find_teacher_word_label_id(gv.STU_ID)  # 老师布置的单词作业id 按时间逆序排列
        system_label_id = self.mysql.find_system_word_label_id(gv.STU_ID)    # 系统单词id
    ...
    @teststep
    def get_word_and_explain_by_group(self, label_homework_bank):
        ...
        for x in bank_id_list:
            if len(x) == 1:
                student_word_list.append(self.mysql.find_student_not_set_word_eql_id(gv.STU_ID, x[0]))
            else:
                student_word_list.append(self.mysql.find_student_not_set_word_in_ids(gv.STU_ID, tuple(x)))
