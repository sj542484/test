...
class ReciteFirstReturnWords(unittest.TestCase):
    ...
    @testcase
    def est_normal_new_operate(self):
                ...
                for x in range(3):
                    if self.word_rebuild.wait_check_game_title_page():
                        ...
                        if ResultPage().wait_check_result_page():
                            all_words = self.word_rebuild.data.get_word_list_by_explains(stu_id, all_explains)    # 获取学生已被解释对应的所有单词
    ...
    def test_normal_recite_operate(self, level):
        if self.home.wait_check_home_page():
            ...
            if self.home.wait_check_home_page():
                ...
                self.word_rebuild.recite_word_operate(stu_id, 1, wrong_again_words, right_explains)  # 复习单词过程
                ...
                if FlashCard().wait_check_study_page():                  # 遇到新词退出
                    self.word_rebuild.from_wordbook_back_to_home()
