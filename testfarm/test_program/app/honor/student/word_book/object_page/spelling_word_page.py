...
class SpellingWord(BasePage):
    ...
    @teststeps
    def dictation_pattern_new(self, i, spell_word):
        """单词默写 新词"""
        if i == 0:
            print('\n单词拼写 - 默写模式(新词)\n')
    ...
    @teststeps
    def dictation_pattern_core(self, spell_word, word_type):
        """单词拼写 - 《默写模式》游戏过程"""
        explain = self.explain()  # 题目
        value = self.common.get_word_by_explain(explain)
        if word_type == 1:
            ...
            if explain == spell_word[0]:
                print('★★★ Error-- 单词默写- 单词出现两次')
            else:
                spell_word[0] = explain

            print('解释：', explain)
            self.key.games_keyboard('backspace')
            print('单词:', value)
            for j in range(0, len(value[0])):
                self.keyboard_operate(j, value[0][j])  # 点击键盘 具体操作
            self.answer_judge()
            ...
            for v in range(len(value)):
                if self.wait_check_explain_page():
                    ...
                    if v == 0:
                        ...
                        self.key.games_keyboard('backspace')
                        print('单词:', value[0])
                    for j in range(0, len(value[v])):
                        self.keyboard_operate(j, value[v][j])  # 点击键盘 具体操作
                    self.answer_judge()
                else:
                    break
    ...
    def hint_ele_operate(self, value):
        ...
        if self.get.enabled(hint) == 'true':
            ...
            if self.dictation_word_judge():  # 出现首字母提示
                ...
                if len(first_word) == 1:
                    if first_word in [x[0] for x in value]:
    ...
    def dictation_random_pattern_recite(self, i):
        ...
        right_word = 0
        for alp in word:
            for x in range(len(list(alp))):
                for y in range(len(list(tip_word))):
                    if x == y and list(tip_word)[y] != '_':
                        if list(tip_word)[y] == list(alp)[x]:
                            compare_list.append(list(tip_word)[y])
            if compare_list == [x for x in list(tip_word) if x != '_']:
                right_word = alp
        alphas = [list(right_word)[x] for x in range(len(list(tip_word))) if list(tip_word)[x] == '_']
        print('缺失的字母：', alphas)
