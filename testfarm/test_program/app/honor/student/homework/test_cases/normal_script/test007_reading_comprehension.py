...
from testfarm.test_program.app.honor.student.homework.object_page.reading_comprehension_page import ReadCompre
...
class Games(unittest.TestCase):
                ...
                if gv.REA_COMPRE in var[0]:  # 该作业存在
                    for i in range(0, len(var[0])):
                        if var[0][i] == gv.REA_COMPRE:
                            var[1][i].click()  # 进入作业
                            count = self.homework.games_count(0, '阅读理解', gv.REA_COMPRE)  # 小游戏个数统计
                    ...
                    print('当前页no have该作业')
                    game = self.home_page.swipe_operate(var[0], gv.REA_COMPRE, '阅读理解')  # 作业list翻页
    ...
    def game_exist(self, count):
        ...
        if len(count) != 0:
            for index in count:
                if self.homework.wait_check_game_list_page(gv.REA_COMPRE):
                    print('####################################################')
