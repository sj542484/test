...
from testfarm.test_program.utils.yb_dict import yb_operate_yb
...
class Games(unittest.TestCase):
        ...
        if self.home_page.wait_check_home_page():  # 页面检查点
            ...
            if self.homework.wait_check_hw_page():  # 页面检查点
                ...
                if gv.MAT_EXE_YB in var[0]:  # 该作业存在
                    ...
                    print('当前页no have该作业')
                    game = self.home_page.swipe_operate(var[0], gv.MAT_EXE_YB, '连连看')  # 作业list翻页
    ...
    def game_exist(self, count):
        ...
        if len(count) != 0:
            for index in count:
                if self.homework.wait_check_game_list_page(gv.MAT_EXE_YB):
                    print('####################################################')
