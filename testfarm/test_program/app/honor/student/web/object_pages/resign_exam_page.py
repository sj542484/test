...
from testfarm.test_program.app.honor.student.web.test_data.teacher_account import TeacherAccount
...
class ResignExamPage(BaseDriverPage):
    ...
    def mine_item_pool(self):
        """我的题库"""
        ele = self.driver.find_element_by_xpath('//*[@id="side-bar"]/div[2]/div/a[2]')
        return ele
        ...
        if self.wait_check_home_page():        # 进入我的试卷
            ...
            while True:                       # 若试题>4, 则删除多余部分， 若等于4 ，则随机布置一个试卷
                if len(self.exam_list()) == 4:
                    self.exam_name(self.exam_list()[random.randint(0, 3)]).click()
