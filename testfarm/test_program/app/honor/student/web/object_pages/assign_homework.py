...
class AssignHomeworkPage(BaseDriverPage):
    ...
    @teststep
    def delete_homework_operate(self):
        self.home.click_test_class()
        # self.home.select_test_class_operate()
        if self.home.wait_check_class_page():
            print('==== 删除已存在test作业 =====\n')
            while True:
                homework_list = self.home.homework_name()
                test_homework_ele = [x for x in homework_list if 'test' in x.text]
                if not [x for x in homework_list if 'test' in x.text]:
                    break
                else:
                    test_homework_ele[0].click()
                    print('删除作业：', test_homework_ele[0].text)
                    time.sleep(1)
                    self.home.homework_tab()[-1].click()
                    time.sleep(1)
                    self.home.delete_btn().click()
                    self.home.tip_box_operate()
                    time.sleep(2)
            ...
            if self.wait_check_bank_pool_page():
                ...
                for x in [('单词练习', '连连看'), ('口语练习', '口语跟读')]:
                    ...
                    if self.wait_check_bank_list_page():
                        ...
                        self.home.join_blanket()
                        self.home.tips_operate()
                        time.sleep(3)


                self.nav_bars()[-1].click()
                if self.home.wait_check_blanket_page():
                    time.sleep(2)
                    self.home.click_select_all()
                    self.home.bank_control_btn()[-1].click()
                    self.home.tip_box_operate()
                    if self.home.wait_check_assign_homework_page():
                        self.homework_name_input_warp().send_keys('test')
                        self.home.select_test_class()
