        ...
        if self.home.wait_check_home_page():
            ...
            if self.home.wait_check_home_page():
                ...
                if self.library.wait_check_library_page(school_name):
                    ...
                    print(course_type)
                    course_name = course_type[1].text                             # 选择指定标签
                    print('选择标签：', course_name)
                    page_set_names = []
                    result = self.data.get_other_set_books(school_id, course_name)  # 获取所有书籍，其他系列书籍
                    all_set_name = result[0]                                      # 改标签下所有系列书籍
                    other_set_name = result[1]                                    # 其他标签书籍
                    course_label_id = result[2]                                   # 标签id
                    self.library.course_more_btn(course_name).click()             # 查看更多
