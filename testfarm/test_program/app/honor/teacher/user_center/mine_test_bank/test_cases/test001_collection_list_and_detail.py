...
from testfarm.test_program.app.honor.teacher.user_center.mine_recommend import RecommendPage
...
from testfarm.test_program.app.honor.teacher.user_center.mine_collection.object_page import CollectionPage
...
class Collection(unittest.TestCase):
        ...
        if self.home.wait_check_page():  # 页面检查点
            ...
            if self.user.wait_check_page():  # 页面检查点
                ...
                if self.collect.wait_check_page():  # 页面检查点
                    ...
                    self.collect_list_page_operation()
    ...
    @teststeps
    def collect_list_page_operation(self):
    ...
    def detail_page_operation(self, name):
        ...
        if self.detail.wait_check_page():  # 页面检查点
            if self.detail.wait_check_list_page():
                var = self.question.question_name()[1][0]  # 获取 小游戏名
                print('-------------------题单详情页 操作-------------------')
                    ...
                    if self.detail.wait_check_list_page():
                        ...
                        if self.detail.wait_check_page():  # 页面检查点
                            ...
                            if self.collect.wait_check_page():
                                self.judge_collect_result(name)  # 验证 - 取消收藏结果
                                self.judge_basket_result(var)  # 验证 - 加入题筐结果
                                self.judge_recommend_result(name)  # 验证 - 推荐结果
    ...
    def judge_collect_result(self, name):
        ...
        if self.collect.wait_check_list_page():  # 是否有收藏
            ...
            if len(name1[0]) != len(name[0]) - 1 or name[1][0] == name1[1][0]:
                print('★★★ Error- 题单详情页 取消收藏失败', len(name1[0]), len(name[0]) - 1)
            else:
                print('取消收藏成功')
        else:
            print('取消收藏成功')
    ...
    @teststeps
    def judge_recommend_result(self, name):
        """ 验证 - 推荐结果"""
        if self.collect.wait_check_page():
            print('--------------验证 加入我的推荐 结果------------')
            ...
            if self.user.wait_check_page():
                ...
                if self.recommend.wait_check_page():
                    if self.recommend.wait_check_list_page():
                        ...
                        if name[1][0] != title[1][0]:
                            print('★★★ Error- 加入我的推荐 失败', name[1][0], title[1][0])
                        else:
                            print(name[1][0])
                            print('加入 我的推荐成功')
