...
class VideoPage(BasePage):
    ...
    video_item_value = 'android:id/title'  # 视频条目
    ...
    def video_file_button(self):
        ...
        print('点击 视频按钮')
        self.driver \
            .find_element_by_xpath("//android.widget.TextView[contains(@text, '视频')]").click()
    ...
    @teststeps
    def video_cut_operation(self, sign='right'):
        """视频 裁剪
        :param sign:前裁剪按钮/后裁剪按钮
        """
        if sign == 'right':
    ...
    @teststeps
    def move_operation(self, origin_x, origin_y, des_x, des_y):
        """移动 元素"""
        TouchAction(self.driver).press(origin_x, origin_y) \
            .move_to(des_x, des_y).release().perform()
