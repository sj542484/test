...
from testfarm.test_program.app.honor.teacher.login.test_data.mine_account import phone_data
        ...
        if self.login.wait_check_page():
            ...
            for i in range(len(phone_data)):
                if self.login.wait_check_page():  # 页面检查点
                    ...
                    phone.send_keys(r'' + phone_data[i]['username'])  # 输入手机号
                    print('账号:', phone.text)

                    pwd.click()  # 激活pwd输入框
                    pwd.send_keys(r'' + phone_data[i]['password'])  # 输入密码
                    print('密码:', pwd.text)
