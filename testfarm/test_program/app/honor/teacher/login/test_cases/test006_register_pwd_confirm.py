...
class Register(unittest.TestCase):
    ...
    def test_register_pwd_confirm(self):
        ...
        # self.login.recovery_data('sffq12a z.w@S万x')  # 恢复测试数据
            ...
            for i in range(len(pwd_data)):
                if self.login.wait_check_page():
                    ...
                    if self.login.wait_check_register_page(3):
                        ...
                        account = ('180243' + str(i) + str(random.randint(1000, 9999)))
                        ...
                        if Toast().find_toast('用户已经注册') or self.login.wait_check_page(3):
                            ...
                            if self.login.wait_check_register_nick_page(3):
                                ...
                                for j in range(index, len(pwd_data)):
                                    ...
                                    if len(pwd_data[j]) == 3:
                                        if Toast().find_toast(pwd_data[j]['assert']):
                                            print(pwd_data[j]['assert'])
                                    else:
