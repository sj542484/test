...
from testfarm.test_program.app.honor.teacher.play_games.object_page.listening_form_sentence import ListenFormSentence
        ...
        if self.home.wait_check_page():  # 页面检查点
            ...
            if self.question.wait_check_page('题单'):  # 页面检查点
                ...
                for i in range(len(name[0])):
                    if name[1][i] == gv.LIS_SENT:
                        name[0][i].click()  # 点击进入该作业

                        count = []  # 小游戏数目
                        self.homework.games_count('听音连句', count)  # 小游戏个数统计
                    ...
                    if self.detail.wait_check_list_page():
                        ...
                        if self.game.wait_check_page():
                            if self.game.wait_check_list_page():
                                ...
                                self.listen.check_detail_page(result2[0])  # 结果页 查看答案
                                print('####################################################')
                                self.homework.back_operation()  # 从结果页返回 题单详情页
        else:
            print('no have听音连句小游戏')
