...
from app.honor.teacher.play_games.object_page.word_reading_page import WordReadPage
...
class Games(unittest.TestCase):
    ...
    @testcase
    def test_word_reading(self):
        self.login.app_status()  # 判断APP当前状态

        if self.home.wait_check_page():
            self.question.search_operation(gv.WOR_RED)  # 进入首页后 进入题库tab，并搜索题单
            self.homework.games_operation(gv.WOR_RED, self.game_exist, '单词跟读')  # 查找题单内 该类型的小游戏
        else:
            ...
            if self.detail.wait_check_list_page():
                ...
                if self.game.wait_check_page():
                    if self.game.wait_check_list_page():
                        ...
                        self.word.word_reading_operation()  # 游戏过程

                        self.word.result_detail_page()  # 结果页
                        self.word.study_again()  # 再练一遍
                        print('#####################################################################')
                        self.word.back_operation()  # 从结果页返回 题单详情页
