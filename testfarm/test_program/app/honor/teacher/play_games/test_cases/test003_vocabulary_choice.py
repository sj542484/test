...
from testfarm.test_program.app.honor.teacher.play_games.object_page.vocabulary_choice_page import VocabularyChoice
        ...
        if self.home.wait_check_page():  # 页面检查点
            ...
            if self.question.wait_check_page('题单'):  # 页面检查点
                ...
                for i in range(len(name[0])):
                    if name[1][i] == gv.VOC_CHO:
                        name[0][i].click()  # 点击进入该作业

                        count = []  # 小游戏数目
                        self.homework.games_count('词汇选择', count)   # 统计小游戏个数
                    ...
                    if self.detail.wait_check_list_page():
                        ...
                        if self.game.wait_check_page():
                            if self.game.wait_check_list_page():
                                ...
                                self.vocab_select.result_detail_page(result2[1][0])  # 结果页 查看答案 按钮
                                print('######################################################')
