...
from app.honor.teacher.play_games.object_page.listening_choice_page import Listening
...
class Games(unittest.TestCase):
    ...
    def test_listen_choice(self):
        ...
        if self.home.wait_check_page():  # 主界面 页面检查点
            self.question.search_operation(gv.LIS_EXE)  # 进入首页后 进入题库tab，并搜索题单

            if self.question.wait_check_page('题单'):  # 页面检查点
                name = self.question.question_name()
                for i in range(len(name[0])):
                    if name[1][i] == gv.LIS_EXE:
                        name[0][i].click()  # 点击进入该作业

                        count = []  # 小游戏数目
                        self.homework.games_count('听后选择', count)  # 小游戏个数统计
                        self.game_exist(count)
                        break
                    ...
                    if self.detail.wait_check_list_page():
                        ...
                        if self.game.wait_check_page():
                            if self.game.wait_check_list_page():
                                ...
                                result = self.listening.listen_choice_operation()  # 听后选择 游戏过程

                                self.result.result_page_star(result)  # 结果页 -- 星星
                                self.listening.study_again()  # 错题再练/再练一遍
                                print('################################################')
