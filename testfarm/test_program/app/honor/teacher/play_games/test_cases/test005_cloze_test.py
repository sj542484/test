...
from testfarm.test_program.app.honor.teacher.play_games.object_page.cloze_test_page import ClozePage
        ...
        if self.home.wait_check_page():  # 页面检查点
            ...
            if self.question.wait_check_page('题单'):  # 页面检查点
                ...
                for i in range(len(name[0])):
                    if name[1][i] == gv.CLO_TEST:
                        name[0][i].click()  # 点击进入该作业

                        count = []  # 小游戏数目
                        self.homework.games_count('完形填空', count)  # 小游戏个数统计
