...
class AssignPaper(unittest.TestCase):
    ...
    def test_assign_paper(self):
        ...
        self.assertTrue(self.question.wait_check_page('试卷'), self.question.question_tips)
        ...
        self.question.judge_into_tab_question('试卷')  # 进入首页后 点击 题库tab
        self.question.filter_button()  # 筛选按钮
        self.assertTrue(self.filter.wait_check_page(), self.filter.filter_tips)
        self.filter.reset_button()  # 重置按钮
        self.filter.commit_button()  # 确定按钮

        if self.question.wait_check_page('题单', 5):  # 页面检查点  由题筐进入；else:  由布置作业按钮 进入
    ...
    def judge_result_operation(self, item, van):
        ...
        if self.van.wait_check_no_hw_page():
            self.assertFalse(self.van.wait_check_no_hw_page(),
                            '★★★ Error-班级动态为空, 布置试卷失败, {}'.format(item))
        else:
