...
class ResultDetailPage(BasePage):
    ...
    def result_voice(self, index):
        """语音按钮"""
        locator = (By.ID, self.voice_value)
        self.wait.wait_find_elements(locator)[index] \
            .click()
    ...
    def result_explain(self):
        """解释"""
        locator = (By.ID, gv.PACKAGE_ID + "tv_hint")
        ele = self.wait.wait_find_elements(locator)
        return ele
    ...
    def result_mine(self):
        """我的"""
        locator = (By.ID, gv.PACKAGE_ID + "iv_mine")
        ele = self.wait.wait_find_elements(locator)
        return ele
    ...
    def judge_voice(self):
        """判断是否存在 发音按钮"""
        return self.wait\
            .wait_check_element(self.voice_value)
    ...
    def question_item(self):
        ...
        item = self.wait.wait_find_elements(locator)
        return item
    ...
    def complete_article_content(self):
        ...
        print(self.wait.wait_find_element(locator).text if self.wait.wait_check_element(locator)
              else '★★★ Error- 文章元素不存在')
    ...
    def dragger_button(self):
        """拖动按钮"""
        locator = (By.ID, gv.PACKAGE_ID + "dragger")
        num = self.wait.wait_find_element(locator)
        return num

    @teststeps
    def options_view_size(self):
        """获取整个选项页面大小"""
        locator = (By.ID, gv.PACKAGE_ID + "option_container")
        num = self.wait.wait_find_element(locator)
        return num.size['height']
    ...
    def all_ears_item(self):
        """磨耳朵"""
        locator = (By.XPATH, "//android.support.v7.widget.RecyclerView/android.widget.LinearLayout/descendant::*")
        ele = self.wait.wait_find_elements(locator)
        ...
        for i in range(len(count) - 1):
            var.append(ele[j] for j in range(count[i], count[i + 1]))
            item.append(var)
    ...
    def option_item(self):
        """选项 内容"""
        locator = (By.ID, gv.PACKAGE_ID + "tv_item")
        ele = self.wait.wait_find_elements(locator)
        return ele
    ...
    @teststeps
    def option_button(self, result):
        """当前页面中所有题目的选项"""
        item = []  # 当前页面中所有题目的选项
        var = []  # 所有题目的正确选项

        count = [i for i in range(0, len(result), 2) if result[i].text == 'A']  # text为A的元素index
    ...
    def hw_detail_operation(self, index, num, achieve=None):
        ...
        if index in (1, 2, 10):  # 单项选择/听后选择/单词听写
            ...
            self.word_swipe_operation(num)  # 具体操作
    ...
    def answer_explain_type(self, num, func, element, content=None):
        ...
        item = (1 if self.judge_voice() else 0)  # 判断是否存在 发音按钮
    ...
    def ears_ergodic_list(self, ques_num, length, var=0):
        ...
        for i in range(var, length):
            count += 1
            print(ques.text for ques in item[i])
            print('--------------------------------')
