...
# -*- coding:utf-8 -*-
# @Author  : SUN FEIFEI
from app.honor.teacher.home.vanclass.object_page.analysis_tab_games_result_detail_page import *
...
class VanclassGameDetailPage(BasePage):
    ...
    drop_down_menu_value = '//span[@class="dialog-content"]'  # 下拉菜单 内容
    ...
    @teststep
    def drop_down_icon(self, var):
        """正确选项后答对率 下拉icon"""
        ele = var.find_element_by_xpath('.//following-sibling::span/span/span[@class="rate-correct-arrow"]')
        self.drop_down(ele)

    @teststep
    def drop_down_text(self, var):
        """正确选项后答对率 下拉按钮text"""
        ele = var.find_element_by_xpath('.//following-sibling::span/span/span')
        return ele

    @teststep
    def drop_down(self, var):
        """正确选项后答对率 下拉操作"""
        loc = ElementBounds().get_element_bounds(var)
        print(loc[2], loc[5] - 10)
    ...
    def drop_down_content(self):
        ...
        for i in range(len(ele)):
            content.append(ele[i].find_element_by_xpath('.//div[@class="article-correct-rate"]').text)
            content.append(ele[i].find_element_by_xpath('.//div[@class="van-cell__title"]/span').text)
        ...
        if self.single.wait_check_list_page():
            ...
            for i in range(len(ques)):
                ...
                for j in range(len(option[0])):
                    print(option[1][j])
                content.append(rate[i].text)  # 答对率
                self.drop_down_operation(rate[i].text, rate[i])  # 下拉菜单内容
                print('---------------------------------')
