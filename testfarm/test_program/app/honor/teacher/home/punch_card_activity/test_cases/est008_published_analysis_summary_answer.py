...
class Activity(unittest.TestCase):
    ...
    def into_activity(self):
        ...
        content = [k for k in range(len(status)) if status[k].text != '已结束']

        index = random.randint(0, len(content) - 1)
        print("进入已发布活动:", name[content[index]].text, status[content[index]].text)
    ...
    def activity_analysis_detail_operation(self, title):
        ...
        self.assertTrue(self.analysis_detail.wait_check_page(title), self.analysis_detail.analysis_vue_tips)
        ...
        self.assertTrue(self.analysis_detail.wait_check_page(title), self.analysis_detail.hw_list_tips)  # 页面检查点
