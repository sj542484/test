...
class VanclassDetailPage(BasePage):
    ...
    van_list_tips = '★★★ Error- 班级详情页无作业'
    van_no_tips = '[用户指南]学生练得不够?给学生布置个作业吧!"]'
    ...
    def wait_check_app_page(self, var):
        ...
        self.my_assert.assertTrue(ele, self.van_tips)
        return ele
    ...
    def wait_check_page(self, var, index=10):
        ...
        self.my_assert.assertTrue(ele, self.van_vue_tips)
        return ele
    ...
    @teststep
    def wait_check_no_hw_page(self, var=3):
        """无作业"""
    ...
    def wait_check_tips_page(self):
        ...
        locator = (By.XPATH, '//div[@class="van-dialog__header"]')
        ele = self.wait.wait_check_element(locator)
        self.my_assert.assertTrue(ele, '★★★ Error- 弹框未弹出')
        return ele
    ...
    def wait_check_school_tips_page(self):
        ...
        ele = self.wait.wait_check_element(locator)
        self.my_assert.assertTrue(ele, '★★★ Error- 未弹 学校名称弹窗')
        return ele
    ...
    # 单词本/每日一听提示信息
    @teststeps
    def wait_check_word_listen_tips_page(self):
        """以“温馨提示”为依据"""
        locator = (By.XPATH, '//div[@class="van-dialog__header" and text()="温馨提示"]')
        ele = self.wait.wait_check_element(locator)
        self.my_assert.assertTrue(ele, '★★★ Error- 未弹 单词本/每日一听提示信息弹窗')
        return ele

    @teststeps
    def word_listen_tips_content(self):
        """ 具体内容"""
        locator = (By.XPATH, '//div[@class="van-dialog__message van-dialog__message--has-title"]')
        item = self.wait.wait_find_element(locator).text
        print(item)
    ...
    def judge_vanclass_result_operation(self, van, assign):
        ...
        :param assign:名称
        """
        self.my_assert.assertTrue(self.home.wait_check_page(), self.home.home_tips)
        print('------------------验证布置结果------------------')
        ...
        for i in range(len(name)):
            ...
            if var == van:
                name[i].click()  # 进入班级
                self.my_assert.assertTrue_new(self.wait_check_app_page(van), self.van_tips)  # 页面检查点
                self.vue.switch_h5()  # 切到vue

                self.my_assert.assertTrue(self.wait_check_page(van), self.van_vue_tips)  # 页面检查点
                if self.wait_check_no_hw_page():
                    self.my_assert.assertTrue(self.wait_check_list_page(), self.van_list_tips)  # 页面检查点
                else:
                    self.my_assert.assertTrue(self.wait_check_list_page(), self.van_list_tips)  # 页面检查点
                    hw = self.hw_name()  # 作业名
                    title = self.home.vanclass_name(hw[0].text)
                    self.my_assert.assertEqual(title, assign, '★★★ Error- 布置作业失败, {} {}'.format(assign, title))
                    print('布置作业成功')
