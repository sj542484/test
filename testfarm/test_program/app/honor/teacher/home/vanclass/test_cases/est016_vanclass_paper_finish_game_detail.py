...
from app.honor.teacher.home.dynamic_info.object_page.paper_detail_page import PaperPage
...
from app.honor.teacher.home.dynamic_info.object_page.paper_finish_tab_student_answer_result_detail_page import ResultDetailPage
...
class VanclassPaper(unittest.TestCase):
    ...
    def finish_situation_operation(self):
        """完成情况tab 具体操作"""
        self.assertTrue(self.paper.wait_check_page(), self.paper.paper_detail_tips)  # 页面检查点
        print('---------------------完成情况tab---------------------')
        if self.paper.wait_check_empty_tips_page():
            self.assertTrue(self.paper.wait_check_empty_tips_page(), '暂无数据')
            print('暂无数据')
        else:
            self.assertTrue(self.paper.wait_check_st_list_page(), self.paper.st_list_tips)
                ...
                if status[i].text == '未完成':
                    ...
                    if self.van_paper.wait_check_page(text):  # 页面检查点
                        ...
                        if self.van_paper.wait_check_page(text):
                            self.van.back_up_button()  # 返回 试卷列表
    ...
    def paper_detail_operation(self, st, content=None):
        ...
        if self.paper.wait_check_paper_list_page():
            ...
            if len(info) > 4 and not content:
                ...
                if self.van_paper.wait_check_page(st):  # 页面检查点
                ...
                if content:
                    for k in range(len(mode)):
                        if content == [mode[k].text, info[k].text]:
                            var = k + 1
                            break
            ...
            if self.van_paper.wait_check_page(st):  # 页面检查点
                if self.paper.wait_check_paper_list_page():
                    ...
                    if self.van_paper.wait_check_page(st):  # 页面检查点
                        ...
                        count = int(re.sub("\D", "", information[0]))  # 小题数
                        score = int(re.sub("\D", "", information[1]))  # 总分数

                        value = game_type_operation(text)
                        mode[i].click()  # 进入游戏 详情页
                        if self.paper.wait_check_per_detail_page(text):
                            if self.paper.wait_check_per_answer_list_page():
                                print(info)
    ...
    def hw_detail_operation(self, index, num, score, achieve=None):
        ...
        if index in (1, 2, 10):  # 单项选择/听后选择/单词听写
            ...
            if self.result.wait_check_match_detail_page():
                self.answer_explain_type(content, self.match.explain_ergodic_list, self.match.result_word, 9)
            else:
                self.answer_explain_type(content, self.match.img_ergodic_list, self.match.result_word, 9)
        elif index == 8:  # 补全文章
            self.complete_article(content)
        elif index == 9:  # 选词填空
            ...
            if self.result.wait_check_answer_list_page():
                self.answer_explain_type(content, self.sentence_trans_ergodic_list, self.trans.result_mine_state)
    ...
    def listen_ergodic_list(self, content, length, var=0):
        ...
        explain = self.listen.result_explain()  # 解释
                    ...
                    if ques_first_index - ques_last_index > 1:  # 判断页面是否滑过，若当前题比上一页做的题不大于1，则下拉直至题目等于上一题的加1
                        for step in range(0, 10):
                            ...
                            if index == ques_last_index + 1:  # 正好
                                ...
                                first_num = self.result.get_first_num(self.result.single_question())
                                if int(first_num) == ques_last_index + 1:  # 正好
                                    ques_num = self.result.single_question()
                                    break
                            # else:
