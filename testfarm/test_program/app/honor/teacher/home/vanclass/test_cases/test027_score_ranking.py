...
class ScoreRanking(unittest.TestCase):
    ...
    def all_score_operation(self):
        ...
        if self.detail.wait_check_empty_tips_page():
            ...
            self.score_operation(4)  # 具体操作

    @teststeps
    def score_operation(self, var=1):
        """积分排行榜页面具体操作"""
    ...
    def compare_operation(self, order, name, num, length, var=1):
        ...
        if order[var].text != "1":   # 第一名 只能与后面同学比较 ，且积分大于其他同学
            if int(num[1].text) < int(num[0].text):
                print('★★★ Error- 积分排行有误：', num[1].text, num[0].text)
            else:
                print('积分排行无误')
        print('------------------------------')
