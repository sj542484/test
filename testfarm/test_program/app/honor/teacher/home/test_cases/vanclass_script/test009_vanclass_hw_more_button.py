...
class Homework(unittest.TestCase):
    ...
    def test_hw_more_button(self):
        ...
        if self.home.wait_check_page():  # 页面检查点
            self.detail.edit_into_operation(gv.VANCLASS, gv.HW_TITLE, self.van.vanclass_hw, gv.HW)  # 进入 班级口语

            if self.hw.wait_check_page():  # 页面检查点
                self.hw.delete_cancel_operation()  # 删除 具体操作
            else:
                print('★★★ Error- 未进入作业包')
