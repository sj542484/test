...
from testfarm.test_program.app.honor.teacher.test_bank.object_page.test_spoken_detail_page import SpokenPage
    ...
    def release_hw_operation(self):
        ...
        if self.release.wait_check_release_page():  # 页面检查点
            if self.release.wait_check_release_list_page():
                ...
                print(name.text)
                name.send_keys(r'' + gv.SPOKEN_ASSIGN)  # 修改name
                ...
                if var == van:
                    ...
                    if self.van.wait_check_page(var):  # 页面检查点
                        if self.van.wait_check_list_page():
                            ...
                            if title != gv.SPOKEN_ASSIGN:
                                print('★★★ Error- 布置作业失败', gv.SPOKEN_ASSIGN, title)
                            else:  # 恢复测试数据
                                print('布置作业成功')
                                hw[0].click()
