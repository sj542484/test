...
class Driver:
    ...
    def run_cases(self,appium_port,sysport):
        ...
        # 开启appium服务
        Utils(port=self.ports).start_appium(dn = self.deviceName, udid=self.udid, plv=self.platformVersion, file_name=file_path,port=appium_port,bp=int(appium_port)+1000,systemPort=sysport)
        # hub地址
        addr = 'http://192.168.8.186:4444/wd/hub'
        # 取保端口已经启动apiumm服务
        while True:
